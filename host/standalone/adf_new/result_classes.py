...
if TYPE_CHECKING:
    from input import OrbitalDefition
...
def CheckIrrepOI(irrepType: Sequence[str]) -> List[Dict[str, str]]:
...
def ReadIrrepOI(complex_result: AMSResults, irrepType: Sequence[str], irrep: str, orb_int_term: float) -> float:
...
def ReadVDD(complex_result: AMSResults, atomList: Sequence[int]) -> List[float]:
    vddList = []
...
def ReadHirshfeld(complex_result: AMSResults, fragment_index: int) -> List[float]:
...
def GetFaIrrep(irrepType: Sequence[str], irrep_orb_number: Sequence[int]) -> List[str]:
...
def GetAtomNum(fragmentList: Mapping[str, Sequence[int]], atoms: Sequence[int]) -> List[int]:
    ...
    original_indices = [atomNum for key in sorted(list(fragmentList.keys())) for atomNum in list(fragmentList[key])]

    reordered_indices = [original_indices.index(i) + 1 for i in atoms]
    return reordered_indices


def GetFragNum(complex_result: AMSResults, frag: str) -> int:
...
def GetFrontIndex(orbSign: str) -> Dict[str, Union[str, int]]:
    """
...
def make_orbital_label(orbital: "OrbitalDefition", include_frag: bool = False) -> str:
    """
...
class PyFragRestrictedResult:
    def __init__(self, complexResult: AMSResults, inputKeys: InputKeys):  # __init__(self, complexJob, inputKeys)
        ...
        for key in list(inputKeys.keys()):
            if key == "overlap" or key == "population" or key == "orbitalenergy" or key == "irrepOI":
                ...
                try:
                    logger.debug(msg="Reading relativistic orbital energies")
    ...
    def GetOrbitalIndex(self, orbDescriptor: "OrbitalDefition"):
        ...
        if GetFrontIndex(orbDescriptor["type"])["holu"] == "HOMO":
            orbIndex = sorted(range(len(self.orbEnergy)), key=lambda x: self.orbEnergy[x] if (self.orbFragment[x] == fragOrbnum) and self.orbOccupation[x] != 0 else -1.0e100, reverse=True)[-int(GetFrontIndex(orbDescriptor["type"])["num"])]
        elif GetFrontIndex(orbDescriptor["type"])["holu"] == "LUMO":
            orbIndex = sorted(range(len(self.orbEnergy)), key=lambda x: self.orbEnergy[x] if (self.orbFragment[x] == fragOrbnum) and self.orbOccupation[x] == 0 else +1.0e100)[int(GetFrontIndex(orbDescriptor["type"])["num"])]
        elif GetFrontIndex(orbDescriptor["type"])["holu"] == "INDEX" and orbDescriptor["index"] is not None:
    ...
    def ReadOverlap(self, index_1, index_2) -> float:
        ...
        faIrrep = GetFaIrrep(self.irrepType, self.irrep_orb_number)
    ...
    def ReadFragorbEnergy(self, index: int) -> float:
    ...
    def GetOutputData(self, complexMolecule: Molecule, outputData: Dict[str, Any], inputKeys: InputKeys):
        ...
        if inputKeys["population"]:
            for i, od in enumerate(inputKeys["population"], start=1):
                ...
                outputData[label] = self.ReadPopulation(self.GetOrbitalIndex(od))
        ...
        if inputKeys["orbitalenergy"]:
            for i, od in enumerate(inputKeys["orbitalenergy"], start=1):
                ...
                outputData[label] = self.ReadFragorbEnergy(self.GetOrbitalIndex(od))

        if inputKeys["irrepOI"]:
            for od in inputKeys["irrepOI"]:
                outputData[f"irrepOI_{od['irrep']}"] = ReadIrrepOI(self.complexResult, self.irrepType, od["irrep"], outputData["OI"])

        if inputKeys["VDD"]:
            outputData["VDD"] = ReadVDD(self.complexResult, GetAtomNum(inputKeys["fragment_indices"], inputKeys["VDD"]))

        if inputKeys["hirshfeld"]:
            outputData["hirshfeld"] = [ReadHirshfeld(self.complexResult, GetFragNum(self.complexResult, fragment_specifier)) for fragment_specifier in inputKeys["hirshfeld"]]
...
class PyFragUnrestrictedResult:
    ...
    def GetOrbnumIndex(self, orbSign: str) -> Dict[str, str]:
    ...
    def GetOrbitalIndex(self, orbDescriptor: "OrbitalDefition") -> Tuple[int, str]:
        # orbDescriptor = {'type' = "HOMO/LUMO/INDEX", 'frag'='#frag', 'irrep'='irrepname', 'index'=i}
        fragOrbnum = GetFragNum(self.complexResult, orbDescriptor["frag"])  # get fragment number
        ...
        # for spin A
        if GetFrontIndex(orbDescriptor["type"])["holu"] == "HOMO":
            orbIndex_AB = sorted(range(len(self.orbEnergy) * 2), key=lambda x: orbEnergy[x] if (orbFragment[x] == fragOrbnum) and orbOccupation[x] != 0 else -1.0e100, reverse=True)[-int(GetFrontIndex(orbDescriptor["type"])["num"])]
            ...
            orbIndex_AB = sorted(range(len(self.orbEnergy) * 2), key=lambda x: orbEnergy[x] if (orbFragment[x] == fragOrbnum) and orbOccupation[x] == 0 else +1.0e100)[int(GetFrontIndex(orbDescriptor["type"])["num"])]
        ...
        elif GetFrontIndex(orbDescriptor["type"])["holu"] == "INDEX":
            spinOrbnum = int(self.GetOrbnumIndex(str(orbDescriptor["index"]))["num"])
            spinOrbspin = self.GetOrbnumIndex(str(orbDescriptor["index"]))["spin"]
            if spinOrbspin == "A":
                for i in range(len(self.orbEnergy)):
    ...
    def ReadOverlap(self, index_1: Tuple[int, str], index_2: Tuple[int, str]) -> float:
        # orbital numbers according to the symmetry of the complex
        faOrb = GetFragOrbNum(self.irrep_orb_number, self.core_orb_number)
        faIrrep = GetFaIrrep(self.irrepType, self.irrep_orb_number)
    ...
    def ReadPopulation(self, index: Tuple[int, str]) -> float:
    ...
    def GetOutputData(self, complexMolecule: Molecule, outputData: Dict[str, Any], inputKeys: InputKeys):
        ...
        if inputKeys["orbitalenergy"]:
            for i, od in enumerate(inputKeys["orbitalenergy"], start=1):
                label = f"orbitalenergy{i}_{make_orbital_label(od, include_frag=True)}"
                outputData[label] = self.ReadFragorbEnergy(self.GetOrbitalIndex(od)[0])
