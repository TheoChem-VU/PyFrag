...
class PyFragResult:
    ...
    def GetOrbnumIndex(self, orbSign) -> Dict[str, str]:
    ...
    def GetOrbitalIndex(self, orbDescriptor):
        ...
        if self.GetFrontIndex(orbDescriptor['type'])['holu'] == 'HOMO':
            orbIndex_AB = sorted(range(len(self.orbEnergy)*2), key=lambda x: orbEnergy[x] if (orbFragment[x] == fragOrbnum)
                                 and orbOccupation[x] != 0 else -1.0E+100, reverse=True)[-int(self.GetFrontIndex(orbDescriptor['type'])['num'])]
            ...
            orbIndex_AB = sorted(range(len(self.orbEnergy)*2), key=lambda x: orbEnergy[x] if (orbFragment[x] == fragOrbnum)
                                 and orbOccupation[x] == 0 else +1.0E+100)[int(self.GetFrontIndex(orbDescriptor['type'])['num'])]
