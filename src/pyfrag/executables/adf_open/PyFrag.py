...
from plams import *
from PyFragModules import WriteDefaultTable, WriteFailFiles, WriteTable, pyfrag_driver
...
parser.add_argument("--fragment1_open_EXTRA", type=str, nargs="*", action="append", help="a file containing adfinput parameters set")
parser.add_argument("--fragment2_open_EXTRA", type=str, nargs="*", action="append", help="a file containing adfinput parameters set")
parser.add_argument("--complex_open_EXTRA", type=str, nargs="*", action="append", help="a file containing adfinput parameters set")
...
for key, val in vars(parser.parse_args()).items():
    if val != None:
        ...
        #    inputKeys[key] = adfComplex + adfFrag1 + adfFrag2
            ...
            adfFrag1_open = ["settings_Frag1_open.input." + adfkey + '="' + keyval.strip("\n") + '"' for adfkey, keyval in adfinputLine]
            adfFrag2_open = ["settings_Frag2_open.input." + adfkey + '="' + keyval.strip("\n") + '"' for adfkey, keyval in adfinputLine]
            adfComplex_open = ["settings_Fa_open.input." + adfkey + '="' + keyval.strip("\n") + '"' for adfkey, keyval in adfinputLine]
            inputKeys[key] = adfGeneral + adfFrag1 + adfFrag2 + adfComplex + adfFrag1_open + adfFrag2_open + adfComplex_open
            ...
            f = open(val[0][0])
            adfinputLine = [(line.split("=", 1)) for line in f.readlines()]
            inputKeys[key] = ["settings_Frag1_open.input." + adfkey + '="' + keyval.strip("\n") + '"' for adfkey, keyval in adfinputLine]

        elif key == "fragment2_open_EXTRA":
            f = open(val[0][0])
            adfinputLine = [(line.split("=", 1)) for line in f.readlines()]
            inputKeys[key] = ["settings_Frag2_open.input." + adfkey + '="' + keyval.strip("\n") + '"' for adfkey, keyval in adfinputLine]

        elif key == "complex_open_EXTRA":
            f = open(val[0][0])
            adfinputLine = [(line.split("=", 1)) for line in f.readlines()]
            inputKeys[key] = ["settings_Fa_open.input." + adfkey + '="' + keyval.strip("\n") + '"' for adfkey, keyval in adfinputLine]
...
tableValues, fileName, failStructures = pyfrag_driver(inputKeys, settings_Frag1, settings_Frag2, settings_Fa, settings_Frag1_open, settings_Frag2_open, settings_Fa_open)
