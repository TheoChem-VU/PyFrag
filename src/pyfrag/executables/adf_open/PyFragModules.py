...
def pyfrag_driver(inputKeys, frag1Settings, frag2Settings, complexSettings, frag1Settings_open, frag2Settings_open, complexSettings_open):
...
class PyFragResult:
    ...
    def get_fragment_orbital_irrep(self):
    ...
    def get_fragment_to_complex_atom_number(self, fragmentList, atoms):
    ...
    def get_fragment_number(self, frag):
    ...
    def split_homo_lumo_index(self, orbSign):
    ...
    def get_fragment_orbital_index(self, orbDescriptor):
        # orbDescriptor = {'type' = "HOMO/LUMO/INDEX", 'frag'='#frag', 'irrep'='irrepname', 'index'=i}
        fragOrbnum = self.get_fragment_number(orbDescriptor["frag"])  # get fragment number
        orbIndex = 0
        if self.split_homo_lumo_index(orbDescriptor["type"])["holu"] == "HOMO":
            orbIndex = sorted(range(len(self.orbEnergy)), key=lambda x: self.orbEnergy[x] if (self.orbFragment[x] == fragOrbnum) and self.orbOccupation[x] != 0 else -1.0e100, reverse=True)[
                -int(self.split_homo_lumo_index(orbDescriptor["type"])["num"])
            ]
        elif self.split_homo_lumo_index(orbDescriptor["type"])["holu"] == "LUMO":
            orbIndex = sorted(range(len(self.orbEnergy)), key=lambda x: self.orbEnergy[x] if (self.orbFragment[x] == fragOrbnum) and self.orbOccupation[x] == 0 else +1.0e100)[int(self.split_homo_lumo_index(orbDescriptor["type"])["num"])]
        elif self.split_homo_lumo_index(orbDescriptor["type"])["holu"] == "INDEX":
    ...
    def read_sfo_population(self, index):
    ...
    def get_vdd_charges(self, atomList):
        vddList = []
    ...
    def get_hirshfeld_charges(self, fragment):
        valueHirshfeld = self.complexResult.readkf("Properties", "FragmentCharge Hirshfeld")
        return valueHirshfeld[self.get_fragment_number(fragment) - 1]

    def get_output_data(self, complexMolecule, outputData, inputKeys):
        ...
        for key, val in list(inputKeys.items()):
            value = []
            if key == "overlap":
                outputData[key] = [self.get_sfo_overlap(self.get_fragment_orbital_index(od1), self.get_fragment_orbital_index(od2)) for od1, od2 in val]

            elif key == "population":
                outputData[key] = [self.read_sfo_population(self.get_fragment_orbital_index(od)) for od in val]

            elif key == "orbitalenergy":
                outputData[key] = [self.get_fragment_orbital_energy(self.get_fragment_orbital_index(od)) for od in val]

            elif key == "irrepOI":
                outputData[key] = [self.get_orbital_interaction_energy(od["irrep"]) for od in val]

            elif key == "hirshfeld":
                outputData[key] = [self.get_hirshfeld_charges(od["frag"]) for od in val]

            elif key == "VDD":
                outputData[key] = self.get_vdd_charges(val["atomList"])

            elif key == "bondlength":
                for od in val:
                    atoms = self.get_fragment_to_complex_atom_number(inputKeys["fragment"], od["bondDef"])
                ...
                for od in val:
                    atoms = self.get_fragment_to_complex_atom_number(inputKeys["fragment"], od["angleDef"])
